    ...
    if result == content:
        ...
        if not overwrite:
            ...
            with open(file, 'w', encoding=write_encoding, errors='ignore') as f:
                f.write(content)
            print(f"\033[32m修改完成！\033[0m已替换的文件路径：{file}")
...
parser.add_argument('replace_content', metavar='<替换内容>', type=str, help='替换后的内容。建议使用引号包裹，由于"$"符号在python中存在其他用途，其原本功能应改用"\\"符号代替')
...
# 判断文件操作方式
replace_fn = replace_content_in_file if os.path.isfile(path) else replace_content_in_dir
replace_fn(path, args.match_content, args.replace_content, args.regex, overwrite)
